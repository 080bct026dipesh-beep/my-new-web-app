...
--
-- Before running, replace every occurrence of /home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/ below with the
...
\copy operators (operator_id, name, service_type, contact_number, rating, unverified_fields) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/operators_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')

-- geom is populated automatically by the trg_stops_set_geom trigger (schema.sql) as each row lands
\copy stops (stop_id, stop_name, aliases, lat, lng, zone, district, ward, is_major_stop, landmark, has_shelter, has_ticket_counter, is_interchange, wheelchair_access, audio_support, status, unverified_fields, created_at, updated_at, geo_out_of_bounds) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/stops_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')

\copy routes (route_id, route_name, short_name, vehicle_type, route_type, operator, start_stop_id, end_stop_id, total_stops, approx_distance_km, estimated_duration_min, service_start_time, service_end_time, frequency_min, fare_type, has_ac, is_express, status, notes, created_at, updated_at, operator_id, return_leg_verified, operator_id_raw, is_multi_operator, haversine_distance_km, max_consecutive_stop_jump_km, approx_distance_km_original, distance_flagged_for_recompute, status_original, status_corrected_for_return_leg, is_bidirectional) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/routes_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')

\copy route_stops (route_id, stop_id, sequence_no) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/route_stops_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')

\copy route_operators (route_id, operator_id, is_primary) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/route_operators_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')
...
-- fare_rules
\copy fare_rules (fare_id, min_distance_km, max_distance_km, fare_npr_min, fare_npr_max, student_discount_pct, verification_note) FROM '/home/djoker_209/Desktop/kathmandu-bus-route-finder/backend/../data/processed/fare_rules_clean.csv' WITH (FORMAT csv, HEADER true, NULL '')
